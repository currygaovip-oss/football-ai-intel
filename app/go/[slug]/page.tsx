...
const kolPages: Record<string, { name: string; title: string; description: string }> = {
  alpha: { name: "AlphaPaw", title: "AlphaPaw 推荐入口", description: "从 AlphaPaw 进入绿茵智报，查看今日足球赛程、赛前分析和赛后复盘。" },
  luobai: { name: "洛白", title: "洛白 推荐入口", description: "从洛白进入绿茵智报，查看今日足球赛程、世界杯赛程和赛前分析。" },
  sevth7: { name: "Sevth7", title: "Sevth7 推荐入口", description: "从 Sevth7 进入绿茵智报，查看今日足球赛程和赛前参考方向。" },
  sanpao: { name: "三炮投研", title: "三炮投研 推荐入口", description: "从三炮投研进入绿茵智报，查看世界杯赛程、今日观点和赛后复盘。" },
  xiaoge: { name: "加密笑哥", title: "加密笑哥 推荐入口", description: "从加密笑哥进入绿茵智报，查看足球赛前观点和复盘记录。" },
  bizixun: { name: "币资讯", title: "币资讯 推荐入口", description: "从币资讯进入绿茵智报，查看赛程、赛前观点和重点赛事信息。" },
  freedom: { name: "Crypto Freedom", title: "Crypto Freedom 推荐入口", description: "从 Crypto Freedom 进入绿茵智报，查看足球赛程和赛前分析。" },
  leiting: { name: "雷霆", title: "雷霆 推荐入口", description: "从雷霆进入绿茵智报，查看世界杯2026赛程和赛前观点。" },
  kongkong: { name: "空空", title: "空空 推荐入口", description: "从空空进入绿茵智报，查看今日足球赛程、参考方向和复盘记录。" }
};
...
export async function generateMetadata({ params }: GoParams): Promise<Metadata> {
  ...
  if (!page) return createMetadata({ title: "绿茵智报推荐入口", description: "绿茵智报推荐入口。", path: `/go/${slug}`, noIndex: true });
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-5 py-10">
      <section className="rounded-lg border border-turf/20 bg-turf/[0.055] p-6 text-center">
        ...
        <h1 className="mt-3 text-3xl font-semibold text-white">{page.name} 推荐入口</h1>
